FactoryBot.define do
  factory :document, parent: :application_records, class: Document do
    ...
    trait :with_notes do
      after(:create) do |document|
        Document.visibilities.keys.each do |visibility|
    ...
    trait :with_3_comments do
      ...
      after(:create) do |document|
        create_list(:comment, 3, commentable: document)
      end
    end
    ...
    trait :with_parent_child_grandchild_comments do
      comments_enabled { true }
      after(:create) do |document|
        comenters = create_list(:user, 3) # Create 3 users to comment
        create_list(:comment, 1, commentable: document, user: comenters.sample).each do |comment|
          create_list(:comment, 1, commentable: comment, user: comenters.sample).each do |reply|
            create_list(:comment, 1, commentable: reply, user: comenters.sample)
          end
        end
      end
    end
    ...
    trait :with_3x3x3_comments do
      ...
      after(:create) do |document|
        comenters = create_list(:user, 3) # Create 3 users to comment
        create_list(:comment, 3, commentable: document, user: comenters.sample).each do |comment|
          create_list(:comment, 3, commentable: comment, user: comenters.sample).each do |reply|
            create_list(:comment, 3, commentable: reply, user: comenters.sample)
          end
        end
      end
    end
    ...
    trait :with_tags do
      after(:create) do |document|
        create_list(:tag, 3, documents: [document])
      end
    end
    ...
    trait :with_content_blocks do
      ...
      after(:create) do |the_document, evaluator|
        evaluator.block_count.times do
          create(
            :content_block,
            "as#{ContentBlock::TYPES.sample}".underscore.to_sym,
    ...
    # Ensure :with_likes trait is also updated as per previous instructions
    trait :with_likes do
      transient do
        likes_count { 2 }
      end
      after(:create) do |page, evaluator|
        create_list(:like, evaluator.likes_count, document: page, user: create(:user))
      end
    end
