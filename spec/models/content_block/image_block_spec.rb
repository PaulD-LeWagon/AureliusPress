...
RSpec.describe ImageBlock, type: :model do
  subject { build(:image_block, :attached_to_a, document_type: :journal_entry) }

  it "is valid and can be persisted" do
    expect(subject.persisted?).to be_falsey
    expect(subject).to be_valid
    expect(subject).to be_a(ImageBlock)
    expect(subject.content_block).to be_valid
    expect(subject.content_block).to be_a(ContentBlock)
    expect(subject.content_block.document).to be_valid
    expect(subject.content_block.document).to be_a(Document)
    expect(subject.content_block.document).to be_a(JournalEntry)
    subject.save!
