...
RSpec.describe VideoEmbedBlock, type: :model do
  ...
  subject { build(:video_embed_block, :with_nil_fields, :attached_to_a, document_type: :blog_post) }
  ...
  context "class methods" do
    describe ".extract_youtube_video_id" do
      it "extracts ID from a standard watch URL" do
        ...
        expect(VideoEmbedBlock.extract_youtube_video_id(url)).to eq("VIDEO_ID876")
      end

      it "extracts ID from a short youtu.be URL" do
        url = "https://youtu.be/VIDEO_ID765"
        expect(VideoEmbedBlock.extract_youtube_video_id(url)).to eq("VIDEO_ID765")
      end

      it "returns nil for a non-YouTube URL" do
        url = "http://www.example.com"
        expect(VideoEmbedBlock.extract_youtube_video_id(url)).to be_nil
      end
