...
RSpec.describe RichTextBlock, type: :model do
  subject { build(:rich_text_block, :attached_to_a, document_type: :page) }
  # For the 'no images' test
  let(:empty_rich_text_block) {
    build(
      :rich_text_block,
      :without_content,
      :attached_to_a, document_type: :page,
    )
  }
  ...
  # it "document type should be BlogPost" do
  #   expect(content_block.document).to be_a(BlogPost)
  # end
  ...
  describe "validations" do
    ...
    it "validates presence of content body (rich text)" do
      ...
      expect(subject).to be_valid
      expect(subject).to be_persisted
      expect(subject).to be_a(RichTextBlock)
