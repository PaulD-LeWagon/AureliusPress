...
RSpec.describe Page, type: :model do
  ...
  subject { create(:page, :with_belt_and_braces, :with_one_of_each_content_block) }
  ...
  context "associations" do
    ...
    it "has dynamically generated associations for contentable types" do
      ...
      expect(subject.content_blocks).to all(be_a(ContentBlock))
    end
  end
  ...
  describe "Delegated Type Content Blocks" do
    it "allows creating content blocks with an ImageBlock contentable" do
      page = create(:page)
      image_content = create(:image_block) # This is the actual image data
      content_block_instance = nil
      expect {
        # Create the ContentBlock that links the page to the specific image_content
        content_block_instance = ContentBlock.create(document: page, contentable: image_content)
    ...
    it "allows creating content blocks with a RichTextBlock contentable" do
      ...
      rich_text_content = build(:rich_text_block)
      content_block_instance = nil
      expect {
        # Create the ContentBlock that links the page to the specific rich_text_content
        content_block_instance = ContentBlock.create(document: page, contentable: rich_text_content)
      }.to change(ContentBlock, :count).by(1)
      expect(content_block_instance).to be_persisted
    ...
    it "creates multiple types of content blocks via trait and retrieves them" do
      ...
      expect(subject.content_blocks.count).to eq(ContentBlock::TYPES.count)
      subject.content_blocks.each do |block|
        expect(block).to be_a(ContentBlock)
        expect(block.contentable).to be_present
        expect(block.contentable).to be_a(ContentBlock::TYPES.map(&:constantize).include(block.contentable.class))
      end
    end
  end
end
