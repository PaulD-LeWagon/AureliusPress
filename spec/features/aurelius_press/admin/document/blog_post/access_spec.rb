...
RSpec.feature "Admin can manage access to Blog Posts", type: :feature, js: true do
  ...
  let!(:posts) { create_list(:aurelius_press_document_blog_post, 3, user: admin) }
  ...
  context "As a user with insufficient permissions - i.e. Reader or User" do
    ...
    scenario "cannot create a new post" do
    ...
    scenario "cannot edit an existing post" do
      [reader, user].each do |the_actor|
        sign_in the_actor
        visit edit_aurelius_press_admin_document_blog_post_path(posts.sample)
        expect(page).to have_content("You are not authorized to perform this action.")
        expect(current_path).to eq(root_path)
        sign_out the_actor
      end
    ...
    scenario "cannot destroy an existing post" do
      [reader, user].each do |the_actor|
        sign_in the_actor
        visit aurelius_press_admin_document_blog_post_path(posts.sample)
        expect(page).to have_content("You are not authorized to perform this action.")
        expect(current_path).to eq(root_path)
        sign_out the_actor
      end
  ...
  context "As a power user: [Moderator, Admin, Superuser]" do
    ...
    scenario "can access the create a new post page" do
      [moderator, admin, superuser].each do |the_agent|
        sign_in the_agent
        visit new_aurelius_press_admin_document_blog_post_path
        expect(page).to have_content("AureliusPress::Admin::Document::BlogPosts#new")
        expect(page).to have_content("Create New Blog Post")
        expect(current_path).to eq(new_aurelius_press_admin_document_blog_post_path)
        sign_out the_agent
      end
    end
    ...
    scenario "can access the edit page" do
      [moderator, admin, superuser].each do |the_agent|
        sign_in the_agent
        visit edit_aurelius_press_admin_document_blog_post_path(posts.last)
        expect(page).to have_content("AureliusPress::Admin::Document::BlogPosts#edit")
        expect(page).to have_content("Edit Blog Post")
        expect(current_path).to eq(edit_aurelius_press_admin_document_blog_post_path(posts.last))
        sign_out the_agent
      end
    end
    ...
    scenario "can access the show page" do
      [moderator, admin, superuser].each do |the_agent|
        sign_in the_agent
        visit aurelius_press_admin_document_blog_post_path(posts.first)
        expect(page).to have_content("AureliusPress::Admin::Document::BlogPosts#show")
        expect(current_path).to eq(aurelius_press_admin_document_blog_post_path(posts.first))
        sign_out the_agent
      end
    end
