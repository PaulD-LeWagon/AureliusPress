...
RSpec.feature "Readers and Users can view Authors (Index, Show)", :js do
  ...
  scenario "READ - Standard Users can view a an author" do
    std_users.each do |the_actor|
      # 1. Log in as a standard user
      sign_in the_actor
      # 2. Navigate to the admin notes index page
      visit aurelius_press_catalogue_authors_path
      # 3. Click on the author's link to view their details
      click_link "View Details", href: aurelius_press_catalogue_author_path(author_1)
      ...
      expect(page).to have_content("Viewing Author")
      expect(page.text.squish).to include(author_1.name.squish)
      # 5. Sign out the user
      sign_out the_actor
    end
  end
