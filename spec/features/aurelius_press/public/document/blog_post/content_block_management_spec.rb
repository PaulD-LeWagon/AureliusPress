...
RSpec.feature "User can manage ContentBlocks on their own BlogPosts", :js, type: :feature do
  # Create a test user
  before do
  ...
  scenario "CREATE - User can create and add a content block to their blog post" do
  ...
  scenario "READ - User can view a blog post with content blocks" do
    ...
    visit aurelius_press_blog_post_path(blog_post_with_content_blocks)
  ...
  scenario "UPDATE - User can update an existing content block" do
    ...
    blog_post_to_update = create(:aurelius_press_document_blog_post, :with_one_of_each_content_block, user: @user)
    # 2. Visit the edit page for the existing content block
    visit edit_aurelius_press_blog_post_path(blog_post_to_update)
