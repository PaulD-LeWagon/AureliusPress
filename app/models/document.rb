class Document < ApplicationRecord
  ...
  COMMENTABLE_TYPES = TYPES - %w[Page]
  ...
  before_validation :check_type_is_set # Or has_many_attached :document_files

  # Associations
  belongs_to :user
  belongs_to :category, optional: true # A Document CAN have a Category
  ...
  has_many :content_blocks, dependent: :destroy, inverse_of: :document
  has_many :taggings, dependent: :destroy # Cascade delete taggings
  has_many :tags, through: :taggings # A Document can have many tags
  has_many :content_blocks, dependent: :destroy # A Document can have many content blocks
  ...
  has_many :comments, as: :commentable, class_name: "Comment", dependent: :destroy, inverse_of: :commentable
  has_many :notes, as: :notable, class_name: "Note", dependent: :destroy, inverse_of: :notable
  # has_many :likes, as: :likeable, class_name: "Like", dependent: :destroy, inverse_of: :likeable
  ...
  def check_type_is_set
    # return if type.present? && TYPES.include?(type)
    # self.type = TYPES.include?(self.class.name) ? self.class.name : throw(:abort)
  end

  # Generate a slug from the title if not provided (correctly uses before_validation)
  def generate_slug
